...
def build_task_notification_body(task, action_label, update_entry=None):
    ...
    if update_entry is not None:
        latest_update_html = f"<p><b>תוכן העדכון החדש:</b><br>{(update_entry.content or '').replace(chr(10), '<br>')}</p>"
    all_updates_html = ''
    if task.updates:
        items = []
        for upd in task.updates:
            items.append(f"<li><b>{upd.author_name}</b> | {upd.created_at.strftime('%d/%m/%Y %H:%M')}<br>{(upd.content or '').replace(chr(10), '<br>')}</li>")
        all_updates_html = '<p><b>כל העדכונים במשימה:</b></p><ul>' + ''.join(items) + '</ul>'
